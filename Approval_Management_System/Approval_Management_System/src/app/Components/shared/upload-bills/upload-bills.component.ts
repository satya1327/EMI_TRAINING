...
export class UploadBillsComponent implements OnInit {
  ...
  data:any;
  uplodBillData: any;
  constructor(private services:RequestServicesService,private fb:FormBuilder,private cd: ChangeDetectorRef,private fileservices:FileUploadService,private toaster:ToastrService,private router:Router) {
  ...
  onSubmit(){
    ...
    this.fileservices.uploadBill(formData).subscribe((response:any) => {
      ...
      this.services.UpdateRequestById(this.userid,response).subscribe(res=>{
        console.log(res);
